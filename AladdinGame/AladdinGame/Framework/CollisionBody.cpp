...
CollisionBody::CollisionBody(GameObject * target)
{
	_target = target;
...
CollisionBody::CollisionBody(GameObject * target, RECT bodyRect)
{
	_target = target;
...
void CollisionBody::checkCollision(GameObject * otherObject, float dt, bool updatePosition)
{
	...
	if (time < 1.0f)
	{
		...
		onCollisionBegin.Emit(otherObject, direction);
	...
	else if (_listColliding.find(otherObject) == _listColliding.end())	// ko có trong list đã va chạm
	{
		...
		{
			onCollisionEnd.Emit(otherObject, direction);
	...
	else	// có trong list đã va chạm
	{
		...
		{
			onCollisionBegin.Emit(otherObject, eDirection::NONE);
...
bool CollisionBody::checkCollision(GameObject * otherObject, eDirection & direction, float dt, bool updatePosition)
{
	float time = isCollide(otherObject, direction, dt);
...
float CollisionBody::isCollide(GameObject * otherSprite, eDirection & direction, float dt)
{
...
bool CollisionBody::isColliding(GameObject * otherObject, float & moveX, float & moveY, float dt)
{
	moveX = moveY = 0.0f;
...
void CollisionBody::updateTargetPosition(GameObject* otherObject, eDirection direction, bool withVelocity, GVector2 move)
{
...
RECT CollisionBody::getSweptBroadphaseRect(GameObject* object, float dt)
{
	// vận tốc mỗi frame
...
eDirection CollisionBody::getSide(GameObject* otherObject)
{
	auto myRect = _target->getBounding();
	auto otherRect = otherObject->getBounding();
...
bool CollisionBody::isColliding(GameObject* otherObject)
{
